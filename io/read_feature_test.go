...
func TestReadWritePolygon(t *testing.T) {
	...
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		newpolys := newfeature.Geometry.(geom.Polygon)
		polys := polygon.Geometry.(geom.Polygon)
		for i, lis := range newpolys.Sublines() {
			for j, pts := range lis.Subpoints() {
				opt := polys.Sublines()[i].Subpoints()[j]
				deltax := math.Abs(pts.X() - opt.X())
				deltay := math.Abs(pts.Y() - opt.Y())
...
func TestReadWriteMultiPolygon(t *testing.T) {
	...
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		newmp := newfeature.Geometry.(geom.MultiPolygon)
		oldmp := multipolygon.Geometry.(geom.MultiPolygon)

		for i, pts := range newmp.Polygons() {
			for j, slins := range pts.Sublines() {
				for k, pt := range slins.Subpoints() {
					opt := oldmp.Polygons()[i].Sublines()[j].Subpoints()[k]
					deltax := math.Abs(pt.X() - opt.X())
					deltay := math.Abs(pt.Y() - opt.Y())
					if deltax > precision || deltay > precision {
...
func TestReadWriteMultiLineString(t *testing.T) {
	...
	newfeature := ReadFeature(bytevals2)
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		newml := newfeature.Geometry.(geom.MultiLine)
		oldml := multilinestring.Geometry.(geom.MultiLine)

		for i, ls := range newml.Lines() {
			for j, ps := range ls.Subpoints() {
				oldps := oldml.Lines()[i].Subpoints()[j]
				deltax := math.Abs(ps.X() - oldps.X())
				deltay := math.Abs(ps.Y() - oldps.Y())
				if deltax > precision || deltay > precision {
...
func TestReadWriteLineString(t *testing.T) {
	...
	newfeature := ReadFeature(bytevals2)
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		newls := newfeature.Geometry.(geom.LineString)
		oldls := linestring.Geometry.(geom.LineString)

		for i, pt := range newls.Subpoints() {
			opt := oldls.Subpoints()[i]
			deltax := math.Abs(pt.X() - opt.X())
			deltay := math.Abs(pt.Y() - opt.Y())
			if deltax > precision || deltay > precision {
...
func TestReadWriteMultiPoint(t *testing.T) {
	...
	newfeature := ReadFeature(bytevals2)
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		newmp := newfeature.Geometry.(geom.MultiPoint)
		oldmp := linestring.Geometry.(geom.MultiPoint)

		for i, pt := range newmp.Points() {
			opt := oldmp.Points()[i]
			deltax := math.Abs(pt.X() - opt.X())
			deltay := math.Abs(pt.Y() - opt.Y())
			if deltax > precision || deltay > precision {
...
func TestReadWritePoint(t *testing.T) {
	...
	newfeature := ReadFeature(bytevals2)
	if newfeature.Geometry.GetType() == newfeature.Geometry.GetType() {
		pt := newfeature.Geometry.(geom.Point)
		opt := point.Geometry.(geom.Point)
